...
class Attention(nn.Module):
    ...
    def forward(
        ...
        weighted_encoder_outputs = torch.bmm(attention, encoder_outputs)
        #weighted_encoder_outputs = attention * encoder_outputs
        c = weighted_encoder_outputs.sum(dim=1)
        #print(c[:, None, :].shape)
        #print(query.shape)
        attn_out = torch.cat([query, c[:, None, :]], dim=-1)
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        for t in range(tgt.shape[1]):
            ...
            output, prev_state = self.lstm(self.embedding(dec_input), prev_state)
            ...
            if t == 0:
                dec_state = prev_state
            else: 
                #print(len(dec_state))
                #print(len(prev_state))
                #print(dec_state[0].shape)
                #print(prev_state[0].shape)
                dec_state = (torch.cat((dec_state[0],prev_state[0]), 0),
                             torch.cat((dec_state[1],prev_state[1]), 0))
                #print(len(dec_state))
                #print(dec_state[0].shape)
